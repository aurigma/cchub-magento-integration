...
namespace Aurigma\CustomersCanvas\Plugin\Minicart;
...
class Image
{
    ...
    public function aroundGetItemData($subject, $proceed, $item)
    {
        ...
        if($projectCollection->getSize()) {
            ...
            if ($projectImageLink) {
                $this->_logger->debug('Image for minicart item (' . $item->getId() . ') was changed.', $this->getLogContext(__METHOD__));
                $result['product_image']['src'] = $projectImageLink;
            }
        } else {
